...
def predict(X,theta):
  ...
  # Calcuate the dot product using np. Could use for loops, but inefficient in Python
  pred=np.dot(X, theta)
  return pred
...
def computeCost(X, y, theta):
  ...
  m = y.size
  divisor_of_cost_function = 2*m
  
  ## cost = sum of (h_theta(x) - y)^2
  cost = sum((predict(X, theta)-y)**2)
  return cost/divisor_of_cost_function
